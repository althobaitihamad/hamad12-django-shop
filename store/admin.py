from django.contrib import admin
from django.utils.html import mark_safe
from .models import Category, Product, ProductImage


class ProductImageInline(admin.TabularInline):
    """عرض الصور الإضافية داخل صفحة المنتج"""
    model = ProductImage
    extra = 1
    verbose_name = "صورة إضافية"
    verbose_name_plural = "📸 الصور الإضافية"
    readonly_fields = ("image_preview",)

    def image_preview(self, obj):
        """عرض مصغّر للصورة في نموذج المنتج"""
        if obj.image:
            return mark_safe(f'<img src="{obj.image.url}" width="70" style="border-radius:6px; border:1px solid #ccc;" />')
        return "لا توجد صورة"
    image_preview.short_description = "معاينة الصورة"


@admin.register(Category)
class CategoryAdmin(admin.ModelAdmin):
    """إدارة التصنيفات في لوحة التحكم"""
    list_display = ("name", "description", "created_at")
    list_display_links = ("name",)
    search_fields = ("name", "description")
    list_filter = ("created_at",)
    ordering = ("-created_at",)
    readonly_fields = ("created_at",)

    fieldsets = (
        ("🗂️ بيانات التصنيف", {
            "fields": ("name", "description"),
            "description": "معلومات أساسية عن التصنيف"
        }),
        ("⏱️ معلومات إضافية", {
            "fields": ("created_at",),
            "description": "تاريخ إضافة التصنيف للنظام"
        }),
    )

    class Meta:
        verbose_name = "تصنيف"
        verbose_name_plural = "التصنيفات"


@admin.register(Product)
class ProductAdmin(admin.ModelAdmin):
    """إدارة المنتجات في لوحة التحكم"""
    list_display = ("name", "category", "price", "stock", "image_preview", "created_at")
    list_display_links = ("name",)
    list_filter = ("category", "created_at")
    search_fields = ("name", "category__name", "description")
    ordering = ("-created_at",)
    readonly_fields = ("created_at", "image_preview")
    inlines = [ProductImageInline]

    fieldsets = (
        ("🛍️ بيانات المنتج", {
            "fields": ("name", "category", "description", "price", "stock", "image"),
            "description": "تفاصيل أساسية عن المنتج"
        }),
        ("📸 معاينة الصورة", {
            "fields": ("image_preview",),
            "description": "عرض مصغّر للصورة الرئيسية"
        }),
        ("⏱️ معلومات إضافية", {
            "fields": ("created_at",),
            "description": "تاريخ إضافة المنتج للنظام"
        }),
    )

    def image_preview(self, obj):
        """عرض مصغّر لصورة المنتج داخل لوحة التحكم"""
        if obj.image:
            return mark_safe(f'<img src="{obj.image.url}" width="70" style="border-radius:6px; border:1px solid #ccc;" />')
        return "لا توجد صورة"
    image_preview.short_description = "معاينة الصورة"

    class Meta:
        verbose_name = "منتج"
        verbose_name_plural = "المنتجات"
